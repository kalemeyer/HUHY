...
export function LiveProjectDirectory({mode}:{mode:'projects'|'maintainers'|'build'}) {
  ...
  const projects=useMemo(()=>records.filter((record)=>['Approved for Incubation','Building','Released'].includes(record.status)&&record.githubUrl),[records]);
